...
const About = ({ DirectorsData, TeachersData, CrewData, base }) => {
  ...
  const user =
    DirectorsData.find((e) => e.username.toLowerCase() == paramuser) ||
    DirectorsData.find((e) => e.username.toLowerCase() == paramuser) ||
    TeachersData.find((e) => e.username.toLowerCase() == paramuser) ||
    TeachersData.find((e) => e.username.toLowerCase() == paramuser) ||
    CrewData.find((e) => e.username.toLowerCase() == paramuser) ||
    CrewData.find((e) => e.username.toLowerCase() == paramuser);
    ...
    <div className="pt-20 cursor-default ">
      ...
      <div className="lg:px-32 md:px-20 lg:py-10 p-5">
          ...
          <div className="lg:w-1/2 h-96">
            <div className="lg:w-11/12 h-full rounded-lg overflow-hidden">
              <div
                ...
                style={{ backgroundImage: `url(${certificate.src}` }}
              ></div>
            </div>
          </div>
        ...
        <div className="w-full flex justify-end z-10 ">
          <div
            className="w-80 h-40 bg-contain bg-no-repeat bg-center -mt-20"
            style={{ backgroundImage: `url(${signature.src}` }}
          ></div>
        </div>
        <div className="w-full flex flex-col pt-20 text-gray-200">
          <div className="lg:w-full font-semibold  flex items-center text-gray-800 mb-10">
            ...
            <p className="mr-5 uppercase lg:text-2xl text-base">
              {employees.directorTitle}
            </p>
          </div>
          {employees.directorText && (
          ...
          <div className="grid lg:grid-cols-4 md:grid-cols-3 grid-cols-2 gap-5 mb-10 ">
            {DirectorsData.map((user, index) => (
              ...
              >
                {user.profilephoto ? (
                  <div
                    className="transition-all duration-500 ease-in-out rounded-lg hover:-translate-y-2 scale-[90%] w-full aspect-1 bg-cover bg-neutral-200 bg-center"
                ...
                <div className="w-full flex flex-col items-center p-5 text-gray-600">
                  <div className="flex font-bold text-center text-sm">
                    <p className="">
                      {user.informations[l].firstname}{" "}
                      {user.informations[l].lastname}
                    </p>
                  </div>
                  <div className="px-3 flex justify-center flex-col items-center text-gray-500 font-thin text-xs rounded-lg text-center">
                    ...
                    <p
                      className="transition-all duration-300 ease-in-out text-xs font-bold text-gray-500 hover:text-gray-400"
                      onClick={() => {
                        setShowTeacher(true);
                        setCurrentTeacher(user);
                        setScrollStop(true);
          ...
          <div className="lg:w-full font-semibold  flex items-center text-gray-800 my-10">
            ...
            <p className="mr-5 uppercase lg:text-2xl text-base">
              {employees.teacherTitle}
            </p>
          </div>
          {employees.teacherText && (
            <p className="text-gray-800 mb-10 text-justify">
              {employees.teacherText}
            </p>
          )}

...
const Admin = ({ api }) => {
  ...
  if (auth.user.artist !== true) {
    return (
      <div className="fixed inset-0 flex justify-center items-center flex-col">
        <p className="">Танд Артист хэсэгт нэвтрэх эрх байхгүй байна!</p>
        <button
  ...
  return (
    <div className="pt-20 cursor-default w ">
        ...
        <div className="w-full grid md:grid-cols-2 md:gap-10 text-gray-600">
          <form className="mt-10 md:pr-10">
            {artistPhoto ? (
              ...
              <div
                className={`transition-all duration-300 ease-in-out aspect-1 w-full bg-cover bg-center rounded-lg mb-10 hover:opacity-80 md:pr-10  flex justify-center items-center ${
                  coverUploaded === true
              ...
              >
                <input
                  ref={fileRef}
                  onChange={(e) => {
                    uploadArtistPhoto(e.target.files[0]);
                  }}
                  multiple={false}
                  type="file"
            ...
            <input
              className="transition-all duration-300 ease-in-out w-full my-2 outline-none border text-sm border-gray-200 rounded-md h-10 px-2 focus:border-sky-500
                font-light mb-5"
              id="username-address"
              name="username"
              type="text"
            ...
            <textarea
              className=" transition-all duration-300 ease-in-out my-2 resize-none text-sm outline-none w-full h-60 border focus:border-sky-500 rounded-md p-2"
              placeholder="Артистын биогоо оруулна уу..."
              spellCheck="false"
              value={bio}
          ...
          <form className="md:mt-10">
            ...
            {auth.user.audios ? (
              <div className="mt-5">
                {auth.user.audios.map((row, index) => (
                  <div
                    key={index}
                    className="w-full h-20  my-2 rounded-md p-2 flex items-center bg-gray-50 hover:bg-gray-100 justify-between"
                  >
                    <div className="flex items-center">
                      <p className="md:mx-8 mx-4 text-lg">{index + 1}</p>
                    ...
                    <div className="text-sm flex items-center">
                      ...
                      <div
                        className="transition-all duration-300 ease-in-out mx-4 rounded-full hover:bg-sky-100 hover:text-sky-500"
                        onClick={() => editAudio({ row, index })}
                      >
                        <svg
            ...
            <button
              className="my-5 transition-all duration-300 ease-in-out w-full h-10  rounded-md font-medium text-white bg-sky-500 hover:opacity-80"
              type="button"
      ...
      {addAudioModal ? (
        <AddAudioModal
          setAddAudioModal={setAddAudioModal}
          setScrollStop={setScrollStop}
          api={api}
        />
      ) : (

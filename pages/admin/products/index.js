...
const Products = () => {
    ...
    const filter = (e, type) => {
        if(type === "title"){
            if(e !=  ''){
                const result = foundproducts.filter(row => {return row.course[l].title.toLowerCase().startsWith(e.toLowerCase())})
            ...
            if(e !=  ''){
              const result = foundproducts.filter(row => {
    ...
    <div className='pt-20 cursor-default'>
      ...
      <div className='w-full lg:px-32 md:px-20 lg:py-10 p-5  '>
        <div className="lg:mb-10 mb-5 flex cursor-default">
          ...
          <p className="transition-all duration-300 ease-in-out text-sm text-black"> Бараанууд </p>
        </div>
        <div className='md:mt-0 mt-10 flex'>
          <div className="transition-all duration-300 ease-in-out hover:text-white hover:bg-green-500 flex items-center px-5 upper bg-green-100 py-2 rounded-md  text-green-500 font-medium "  onClick={()=> router.push("/admin/products/new")}>
              ...
              <tr>
                ...
                <th scope="col" className="px-6 py-3">
                  <select className='py-2 outline-none'>
                    ...
                    {/* {foundProducts && foundProducts.map((row, index)=> (
                        <option key={index} value={row.type}>
                            {row.type}
                        </option>
                    ))}*/}
              ...
              foundProducts.map((row, index) => (
                ...
                {row.cover ?
                  ...
                  <td scope="row" className=" font-medium text-gray-900">
                    <div className='h-20 w-20 flex bg-gray-100 justify-center items-center m-2 rounded-sm' onClick={()=> router.push("/admin/products/"+row._id)}>
                  ...
                  <td scope="row" className='px-6 py-3'>
                    <p className='hover:underline' onClick={()=> router.push("/admin/products/"+row._id)}>
                    {row.title}
                    </p>
                  </td>
                  <td  scope="row" className='px-6 py-3'>
                  <p className='hover:underline' onClick={()=> router.push("/admin/products/"+row._id)}>
                    {row.type === "guitar" ? "Гитар" : row.type === "ukulele" ? "Үкүлэлэ" : "Бусад"}
                    </p>
                  </td>
                  <td  scope="row" className='px-6 py-3'>

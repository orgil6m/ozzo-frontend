    ...
    if (type === "title") {
      if (e != "") {
        const result = foundCourses.filter((row) => {
          return row.course[l].title.toLowerCase().startsWith(e.toLowerCase());
        });
      ...
      if (e != "") {
        const result = foundCourses.filter((row) => {
          if (e == "true" && row.individual === true) {
            ...
            e == "false" &&
            (row.individual == undefined || row.individual == "")
          ) {
            return row;
          }
        });
    ...
    <div className="pt-20 cursor-default">
      ...
      <div className="w-full lg:px-32 md:px-20 lg:py-10 p-5  ">
        ...
        <div className="md:mt-0 mt-10 flex">
          <div
            className="transition-all duration-300 ease-in-out hover:text-white hover:bg-green-500 flex items-center px-5 upper bg-green-100 py-2 rounded-md  text-green-500 font-medium "
            ...
            <thead className="text-xs text-gray-800 uppercase bg-gray-100 ">
              <tr>
                <th scope="col" className="px-6 py-3">
                  <p className="truncate">Зураг</p>
                </th>
                <th scope="col" className="px-6 py-3">
                  <p className="truncate">Хөтөлбөрийн нэр</p>
                </th>
                <th scope="col" className="px-6 py-3">
                  <p className="truncate">Ганцаарчилсан</p>
                </th>
                <th scope="col" className="px-6 py-3">
                  <p className="truncate">Групп</p>
                </th>
                <th scope="col" className="px-6 py-3">
                  <p className="truncate">Оруулсан</p>
                </th>
                <th scope="col" className="px-6 py-3">
                  <p className="truncate">Сүүлд зассан</p>
                </th>
              </tr>
            </thead>
            <thead className="text-xs text-gray-800 uppercase border-b ">
              <tr>
                ...
                <th scope="col" className="px-2 py-3">
                  <input
                    className="transition-all duration-300 ease-in-out outline-none pl-4 py-2 rounded-sm border border-gray-100 focus:border-gray-300"
                    placeholder="Хайх Утга..."
                    onChange={(e) => filter(e.target.value, "title")}
                  />
                </th>
                <th scope="col" className="px-6 py-3">
                  <select
                    className="py-2 outline-none "
                    onChange={(e) => filter(e.target.value, "ind")}
                  >
                    <option value={""}>Шүүх</option>
              ...
              {foundCourses && foundCourses.length > 0 ? (
                foundCourses.map((row, index) => (
                  <tr
                    ...
                    className={`border-b ${
                      select === index ? "text-gray-900 bg-gray-100" : ""
                    }`}
                  >
                    {row.img ? (
                      <td
                        scope="row"
                        className="w-20 font-medium text-gray-900 "
                      >
                        <div
                          className="h-20 w-20 bg-cover bg-center m-2 rounded-sm"
                          style={{ backgroundImage: `url(${row.img}` }}
                          onClick={() =>
                            router.push("/admin/courses/" + row._id)
                          }
                        ></div>
                      </td>
                    ) : (
                      <td scope="row" className=" font-medium text-gray-900">
                        <div
                          className="h-20 w-20 flex bg-gray-100 justify-center items-center m-2 rounded-sm"
                          onClick={() =>
                            router.push("/admin/courses/" + row._id)
                          }
                        >
                    ...
                    <td scope="row" className="w-20 text-gray-700 px-6 py-3">
                      <p className="truncate">
                        {" "}
                        {row.updatedDate && moment(row.updatedDate).calendar()}
                      </p>
                    </td>
                  </tr>
                ))

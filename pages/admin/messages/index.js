...
export async function getServerSideProps(context){
    ...
    if(query) {
        ...
        return {
            props: {data, selectedMessage, query},
        }
    }
    return {
        props: {data},
...
const Messages = ({data, selectedMessage, query}) => {
    ...
    useEffect(() => {
        ...
        if(selectedMessage) setSelected({row : selectedMessage, index : query})
    }, [])
    ...
    const deleteMessage = async (index) => {
        ...
        const response = await fetch(`${process.env.API_URL}/api/ozzo/message`,{
          method: "DELETE",
        ...
        <div className='pt-20 cursor-default bg-white'>
            ...
            <div className='w-full lg:px-32 md:px-20 p-5  '>
                <div className="lg:mb-10 mb-5 flex cursor-default md:mt-5">
                    ...
                    <p className="transition-all duration-300 ease-in-out text-sm pr-2 text-black" onClick={() => router.push("/admin/users")}> Санал Хүсэлтүүд </p>
                </div>
                ...
                <div className='md:block hidden p-5 border border-gray-100 rounded-md pb-10'>
                    <div className='w-full flex justify-between'>
                        ...
                        <div className='flex'>
                            ...
                            <svg className="mr-2 transition-all duration-100 ease-in-out h-4 w-4 hover:text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} onClick={()=> deleteMessage(selected.row._id)}>
                    ...
                    <div className='flex items-center mt-5'>
                        <div className='rounded-full text-white flex h-10 w-10 justify-center items-center' style={{backgroundColor: selected.row.avatarColor}}>
                            <span className='m-2'>
                            {selected.row.username.slice(0, 1)}
                            </span>
                        </div>
                        <span className='mx-3 flex flex-col items-start justify-center font-medium'>
                            {selected.row.username} 
                            <a className='text-xs text-gray-500 flex items-center hover:underline font-light' href={`mailto:${selected.row.email}`}>
            ...
            {showMessage ? <ShowMessage message={selected.row} setShowMessage={setShowMessage}/> : <></>}
        </div>
    )
}

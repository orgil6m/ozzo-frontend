...
import cover from "../../Assets/STORE/store_cover.jpg";
...
const nexg = [
  {
    title: ["NEXG ягаан", "NEXG pink", "NEXG粉红"],
  ...
  {
    title: ["NEXG цэнхэр", "NEXG blue", "NEXG蓝"],
  ...
  {
    title: ["NEXG цагаан", "NEXG white", "NEXG白"],
  ...
  {
    title: ["NEXG хар", "NEXG black", "NEXG黑色"],
...
const Store = ({ GuitarsData, UkulelesData }) => {
  ...
  return (
    <div className="pt-20 ">
      ...
      <div className="lg:px-32 md:px-20 lg:mt-10 p-5">
        ...
        <div className="w-full grid md:grid-cols-4 grid-cols-2 md:gap-10 gap-3 mb-10">
          {nexg.map((row, index) => (
            <div
              key={index}
              className="transition-all duration-300 ease-in-out border border-gray-100 w-full flex flex-col justify-center items-center rounded-md md:hover:-translate-y-2 hover:border-gray-200 relative"
            >
      ...
      <div
        className="w-full md:bg-fixed bg-bottom bg-cover"
            ...
            >
              ...
              <div className="absolute top-3 left-3 bg-white rounded-full flex">
                {row.colors.map((color, i) => (
                  <div
                    key={i}
                    className={`md:h-4 md:w-4 h-3 w-3 rounded-full mr-1  ${color.class}`}
                  ></div>
                ))}
              </div>
            </div>
      ...
      <div
        ...
        style={{ backgroundImage: `url(${cover4.src}` }}
      >
        <div className="p-32 w-full h-[30rem] flex justify-center items-center"></div>
      </div>

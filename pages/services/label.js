...
import cover from "../../Assets/LABEL/label_cover.jpg";
                ...
                {label.services.map((service, index) => (
                  <div key={index} className="flex items-start">
                    <div className="text-red-500 ">
                      <svg
                        className="h-5 w-5 mr-2 mt-2 m-1"
                        viewBox="0 0 20 20"
                        fill="currentColor"
                      >
                        <path
                    ...
                    <p className="my-2 text-sm text-justify">{service}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="grid lg:grid-cols-4 md:grid-cols-3 grid-cols-2 lg:gap-10 gap-5">
          {ArtistData.map((user, index) => (
            <div
              key={index}
              onClick={() => {
                router.push(`/artists/${user.artistName}`);
              }}
            >
              {user.artistPhoto ? (
                <div
                  className="transition-all duration-500 ease-in-out rounded-md hover:-translate-y-2 w-full aspect-1 bg-cover bg-neutral-200 bg-center"
              ...
              <div className="w-full flex flex-col items-center pt-3 text-gray-700/90">
                ...
                <div className="px-3 flex justify-center flex-col items-center text-gray-500 font-thin md:text-xs rounded-lg text-center">
                  <p></p>
                  <p
                    className="transition-all duration-300 ease-in-out font-bold text-gray-500 pt-1 hover:text-gray-400"
                    onClick={() => {
                      setShowTeacher(true);

...
export default function Example() {
    ...
    <div className="w-56 text-right fixed top-16">
      <Menu as="div" className="relative inline-block text-left">
        <div>
          <Menu.Button className="inline-flex justify-center w-full px-4 py-2 text-sm font-medium text-white bg-black rounded-lg bg-opacity-90 hover:bg-opacity-80 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 w-20">
            {t[0]}
            <ChevronDownIcon
              ...
              <Menu.Item>
                ...
                {({ active }) => (
                  ...
                  onClick={() => { router.push({ pathname, query }, asPath, { locale: t[1] })}} >
                    {active ? ( <EditActiveIcon className="w-5 h-5 mr-2" aria-hidden="true" />) : (
                ...
                {({ active }) => (
                  <button className={`${ active ? 'bg-violet-500 text-white' : 'text-gray-900' } group flex rounded-md items-center w-full px-2 py-2 text-sm`} 
                   onClick={() => { router.push({ pathname, query }, asPath, { locale: t[2] })}} >
                    {active ? ( <DuplicateActiveIcon className="w-5 h-5 mr-2" aria-hidden="true" /> ) : ( 

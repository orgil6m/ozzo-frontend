...
const Audioplayer = ({data, audioIndex, setAudioIndex, userCover, artist}) =>{
    ...
    <div className='w-screen h-28 left-0 bottom-0 fixed lg:z-30 z-20 flex items-end justify-center bg-neutral-900 backdrop-blur-lg border-t-2 border-white/20'> 
          <div className=" h-full md:flex hidden items-center md:pl-20 pl-10">
            <div className="h-16 w-16 bg-cover bg-center rounded-lg" style={{'backgroundImage': `url(${artist.artistPhoto}`}}>
            </div>
            <div className="flex flex-col w-40">
              <p className="px-5 capitalize text-gray-100 truncate" >
                {data[audioIndex].title}
              </p>
              <p className="px-5 text-gray-100/50 text-sm"> 
                {artist.artistName}
              </p>
            </div>
          </div>
            ...
            <div className="flex items-center text-sm text-gray-100">
              {new Date(Math.floor(trackProgress) * 1000).toISOString().slice(15, 19)}
               <input
                    type="range"
                    value={trackProgress}
                    step="any"
      ...
      <div className="hidden h-full md:w-1/4 md:flex items-center justify-center">
        <div className="w-32 h-10 flex items-center ">
          {volume <= 0 ?
          <img className="w-4 opacity-80 mr-2 " src={volume0.src} alt="volume" />  
          :
          volume < 25 ?
          <img className="w-4 opacity-80 mr-2" src={volume1.src} alt="volume" />  
          :
          volume < 50 ?
          <img className="w-4 opacity-80 mr-2" src={volume2.src} alt="volume" />  
          :
          <img className="w-4 opacity-80 mr-2" src={volume3.src} alt="volume" />  
        }
          <input
            type="range"
            value={volume}
